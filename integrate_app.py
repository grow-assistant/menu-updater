...
from utils.create_sql_statement import (
    generate_sql_from_user_query,
    load_schema,
    create_prompt,
    generate_sql_with_custom_prompt,
)
...
sys.path.append(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "tests", "simulations")
)
...
class CategorizationResult(BaseModel):
    request_type: str = Field(
        ..., description="Type of request from predefined categories"
    )
    time_period: Optional[str] = Field(
        None, description="Time period for order history queries"
    )
    analysis_type: Optional[str] = Field(
        None, description="Type of analysis being requested"
    )
    ...
    item_name: Optional[str] = Field(
        None, description="Menu item name for update/enable/disable requests"
    )
    new_price: Optional[float] = Field(
        None, description="New price for menu items", ge=0
    )
...
def demonstrate_complete_flow(
    test_query=None, previous_context=None, persona=None, voice=None, mock_session=None
):
    """
    ...
    try:
        ...
        if not context_files:
            print(
                "❌ Error: Failed to load context files. Using fallback empty contexts."
            )
            context_files = {
                "business_rules": "",
                "database_schema": "",
        ...
        categorization_response = openai_client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"Analyze this query and return a JSON response: {test_query}",
                },
            ],
        ...
        if categorization.start_date and categorization.end_date:
            ...
            print(
                f"  Updated date cache: {categorization.start_date} to {categorization.end_date}"
            )
        # Clear date cache if a new time period is mentioned but no specific dates provided
        elif categorization.time_period and categorization.time_period.lower() not in [
            "",
            "none",
            "no specific period",
            "not specified",
            ...
            print(
                f"  Cleared date cache due to new time period: {categorization.time_period}"
            )
        elif mock_session.date_filter_cache:
            result_context["date_filter"] = mock_session.date_filter_cache
            print(
                f"  Using cached dates: {mock_session.date_filter_cache['start_date']} to {mock_session.date_filter_cache['end_date']}"
            )
        ...
        try:
            ...
            date_context_obj = categorization_result.get("context", {}).get(
                "date_context", None
            )
            ...
            if is_order_history:
                print(
                    "  Using PATH 1: Enhanced Gemini prompt with full business context"
                )
                ...
                if query_type == "update_price":
                    ...
                    if len(mock_session.selected_location_ids) > 1:
                        locations_str = ", ".join(
                            map(str, mock_session.selected_location_ids)
                        )
            ...
            if not continue_to_execution:
                try:
                    ...
                    time_period = (
                        categorization.time_period
                        if hasattr(categorization, "time_period")
                        else None
                    )
        ...
        try:
            ...
            if execution_result.get("success"):
                ...
                if result_count > 0:
                    ...
                    displayed_result = {
                        k: v for k, v in first_result.items() if len(str(v)) < 50
                    }
        ...
        try:
            if execution_result and execution_result.get("results"):
                ...
                summarization_prompt = create_summarization_prompt(
                    execution_result["results"], test_query, verbal_history
                )
...
"""
                        ...
                        text_match = re.search(
                            r"TEXT_ANSWER:(.*?)$", raw_response, re.DOTALL
                        )
                        if text_match:
                        ...
                        if attempt < max_retries:
                            print(
                                f"  Retry {attempt+1}/{max_retries} after error: {str(retry_error)}"
                            )
                        else:
                            raise
        ...
        verbal_answer = (
            flow_steps.get("summarization", {})
            .get("data", {})
            .get("verbal_answer", summary)
        )
        text_answer = (
            flow_steps.get("summarization", {})
            .get("data", {})
            .get("text_answer", summary)
        )
        ...
        query_context = {
            ...
            "conversation_history": conversation_history,
            "date_context": categorization_result.get("context", {}).get(
                "date_context", None
            ),  # Include date_context from categorization
            "verbal_history": (
                conversation_history[-3:] if len(conversation_history) >= 3 else []
            ),
        }
        ...
        return {
            ...
            "steps": flow_steps,
            "context": (
                query_context if "query_context" in locals() else {"error": str(e)}
...
def call_sql_generator(
    ...
    from prompts.google_gemini_prompt import (
        create_gemini_prompt as external_gemini_prompt,
    )
...
def create_categorization_prompt(cached_dates=None):
    ...
    from prompts.openai_categorization_prompt import (
        create_categorization_prompt as external_create_prompt,
    )
...
def setup_elevenlabs() -> bool:
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                "Skipping client API due to known ArrayJsonSchemaProperty issue"
            )
            ...
            try:
                ...
                print(
                    "✓ ElevenLabs package (older version) loaded successfully with legacy approach"
                )
                return True
...
class ElevenLabsVoice:
    ...
    def __init__(
        self, default_voice_id: str = "UgBBYS2sOqTuMpoF3BR0", persona: str = None
    ):
        self.enabled = True
        ...
        try:
            ...
            if USE_NEW_API:
                ...
                try:
                    ...
                    if not voice_exists:
                        print(
                            f"⚠️ Specified voice ID {self.voice_id} not found, defaulting to first available voice"
                        )
                ...
                try:
                    ...
                    voice_exists = any(
                        getattr(voice, "voice_id", None) == self.voice_id
                        for voice in self.available_voices
                    )
                    if not voice_exists and self.available_voices:
                        ...
                        self.voice_id = getattr(
                            self.available_voices[0], "voice_id", self.voice_id
                        )
    ...
    def ensure_concise(self, text: str) -> str:
        ...
        print(
            f"Verbal response truncated from {len(words)} to {self.max_verbal_length} words for speech"
        )
        return concise_text
                ...
                try:
                    ...
                    if os.name == "nt":  # Windows
                        ...
                        os.system(
                            ...
                            else f"xdg-open {temp_path}"
                        )

                    print(
                        f"🔊 Playing audio with system default player from {temp_path}"
                    )
                    return True
                except Exception as e:
    ...
    while True:
        try:
                        ...
                        if 0 <= persona_idx < len(AVAILABLE_PERSONAS):
                            ...
                            if voice and voice.initialized:
                                ...
                                if voice_id:
                                    ...
                                    print(
                                        f"Voice updated for {selected_persona} persona"
                                    )
                        else:
                            print(
            ...
            if result.get("success"):
                ...
                response = (
                    f"Error processing query: {result.get('error', 'Unknown error')}"
                )
...
def run_streamlit_app():
        ...
        if (
            ...
            with st.spinner(f"Checking ..."):
                ...
                if result.get("success"):
                    ...
                    text_answer = result.get("context", {}).get(
                        "previous_text_answer",
    ...
    with st.sidebar.expander("Location Settings", expanded=True):
        ...
        st.caption(
            "Choose a country club to view data from. If a club has multiple locations, you can view data from a specific location or all locations."
        )
        ...
        selected_club = st.selectbox(
            ...
            options=club_names,
            index=(
                club_names.index(st.session_state.selected_club)
                if st.session_state.selected_club in club_names
                else 0
            ),
        )
        ...
        if len(locations_for_club) > 1:
            ...
            for loc_id in locations_for_club:
                ...
                if "(" in location_options[loc_id]:
                    location_label = (
                        location_options[loc_id].split("(")[1].replace(")", "").strip()
                    )
                else:
            ...
            if (
                len(st.session_state.selected_location_ids) == 1
                and st.session_state.selected_location_ids[0] in locations_for_club
            ):
            ...
            location_selection = st.radio(
                ...
                format_func=lambda x: next(
                    (opt[1] for opt in location_options_list if opt[0] == x), x
                ),
                index=(
                    [opt[0] for opt in location_options_list].index(current_selection)
                    if current_selection in [opt[0] for opt in location_options_list]
                    else 0
                ),
            )
    ...
    with st.sidebar.expander("Persona Settings", expanded=True):
        selected_persona = st.radio(
            ...
            index=(
                AVAILABLE_PERSONAS.index(st.session_state.persona)
                if st.session_state.persona in AVAILABLE_PERSONAS
                else 0
            ),
        ...
        if selected_persona != st.session_state.persona:
            ...
            if (
                st.session_state.voice_instance
                and st.session_state.voice_instance.initialized
            ):
                result = st.session_state.voice_instance.update_persona(
                    selected_persona
                )
                st.success(
                    f"Persona set to {selected_persona} with voice update: {result}"
                )
            else:
            ...
            if voice_status["elevenlabs"] and voice_status["audio_playback"]:
                ...
                if voice.available_voices:
                    ...
                    selected_voice = st.selectbox(
                        "Select voice", options=voice_options, index=current_index
                    )
                ...
                st.warning(
                    "Voice output unavailable. Install required packages: elevenlabs and pygame"
                )
                st.session_state.voice_instance = None
    else:
    ...
    st.write(
        "Ask any question about your restaurant data. I'll analyze your query and provide insights."
    )
    ...
    if len(st.session_state.selected_location_ids) == 1:
        ...
        st.info(
            f"📍 Currently querying data for: **All {st.session_state.selected_club} Locations**"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Run in interactive terminal mode"
    )
