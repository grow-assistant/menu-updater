...
def demonstrate_flow_with_langchain(
    ...
    if not use_langchain:
        ...
        return demonstrate_complete_flow(test_query=test_query, previous_context=previous_context)
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Run the demo with LangChain integration")
    parser.add_argument("--query", type=str, help="Query to process", default=None)
    parser.add_argument("--original", action="store_true", help="Use original flow instead of LangChain")
    args = parser.parse_args()
    
    # Process the query
    result = demonstrate_flow_with_langchain(
        test_query=args.query,
        use_langchain=not args.original
    )
