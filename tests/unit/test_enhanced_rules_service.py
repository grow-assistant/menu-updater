...
def rules_service(mock_config, mock_yaml_loader):
    ...
    with patch("services.rules.rules.get_yaml_loader") as mock_get_loader:
...
class TestEnhancedRulesService:
    ...
    def test_initialization(self, rules_service, mock_config):
        ...
        assert rules.rules_path == mock_config["services"]["rules"]["rules_path"]
        assert rules.resources_dir == mock_config["services"]["rules"]["resources_dir"]
        assert rules.cache_ttl == mock_config["services"]["rules"]["cache_ttl"]
        assert isinstance(rules.cached_rules, dict)
        assert isinstance(rules.cache_timestamps, dict)
    ...
    def test_get_rules_and_examples_with_cache(self, rules_service):
        ...
        rules.base_rules = {category: test_rules}
        
        # First call should process and cache
        result = rules.get_rules_and_examples(category)
        assert result == test_rules
        assert category in rules.cached_rules
        
        # Second call should use cache
        with patch.object(rules_service, "_process_rules_for_category") as mock_process:
            result2 = rules.get_rules_and_examples(category)
    ...
    def test_get_rules_and_examples_unknown_category(self, rules_service):
        ...
        result = rules.get_rules_and_examples("unknown_category")
    ...
    def test_invalidate_cache_specific_category(self, rules_service):
        ...
        rules.cache_timestamps = {
            "category1": 12345,
            "category2": 67890
        }
        
        # Invalidate one category
        rules.invalidate_cache("category1")
        
        # Check that only the specified category was removed
        assert "category1" not in rules.cached_rules
        assert "category1" not in rules.cache_timestamps
        assert "category2" in rules.cached_rules
        assert "category2" in rules.cache_timestamps
    ...
    def test_invalidate_cache_all_categories(self, rules_service):
        ...
        assert len(rules.cached_rules) == 0
        assert len(rules.cache_timestamps) == 0
    
    def test_get_sql_patterns(self, rules_service, mock_yaml_loader):
        """Test getting SQL patterns."""
        pattern_type = "menu"
        result = rules.get_sql_patterns(pattern_type)
    ...
    def test_get_schema_for_type(self, rules_service):
        ...
        with patch.object(rules_service, "get_sql_patterns") as mock_get_patterns:
            ...
            result = rules.get_schema_for_type("test_type")
    ...
    def test_get_sql_pattern(self, rules_service):
        ...
        with patch.object(rules_service, "get_sql_patterns") as mock_get_patterns:
            ...
            result = rules.get_sql_pattern("test_type", "test_pattern")
    ...
    def test_format_rules_for_prompt(self, rules_service):
        ...
        result = rules.format_rules_for_prompt(rules)
