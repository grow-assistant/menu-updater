...
def get_location_settings(connection, location_id):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT settings FROM locations WHERE id = %s", (location_id,)
            )
...
def update_location_settings(connection, location_id, settings):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE locations SET settings = %s WHERE id = %s",
        ...
        with connection:  # Auto-commits or rolls back
            with connection.cursor() as cursor:
                ...
                if "UPDATE items" in query.lower():
                    ...
                    cursor.execute(
                        "SELECT * FROM items WHERE id = %s FOR UPDATE", (item_id,)
                    )
                ...
                if "UPDATE categories" in query.lower() and (
                    "start_time" in query or "end_time" in query
                ):
                    time_matches = re.findall(
                        r"(?:start|end)_time\s*=\s*(\d{4})", query, re.IGNORECASE
                    )
                ...
                if operation_name and "location_id" in st.session_state:
                    settings = get_location_settings(
                        connection, st.session_state["location_id"]
                    )
                    settings = add_operation_to_history(
                        settings,
                        {
                    ...
                    update_location_settings(
                        connection, st.session_state["location_id"], settings
                    )
...
def get_location_hours(connection, location_id: int):
    ...
    return execute_template_query(
        connection, "view_location_hours", {"location_id": location_id}
    )


def update_location_hours(
    connection, location_id: int, day_of_week: str, open_time: str, close_time: str
):
...
def get_markers(connection, location_id: int):
    ...
    return execute_template_query(
        connection, "view_markers", {"location_id": location_id}
    )


def add_marker(connection, marker_data: dict):
    """Add marker using template"""
    return execute_template_query(connection, "insert_marker", marker_data)
...
def process_query_results(results):
    ...
    context = {"query": results["query"], "success": results["success"]}
    ...
    try:
        ...
        model = st.secrets.get("MODEL_FOR_ANALYSIS", "gpt-3.5-turbo")
        temp = float(st.secrets.get("ANALYSIS_TEMPERATURE", "0.3"))

        # Create system prompt
        system_prompt = f"""You are a helpful assistant that converts database \
{result_type} into natural language responses.
