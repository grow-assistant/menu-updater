...
def create_langchain_agent(
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Failed to initialize agent: {str(e)}. Inner error: {str(inner_e)}")
    ...
    try:
        ...
        try:
            ...
            categorization_response = openai_client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"Analyze this query and return a JSON response: {query}"}
                ],
            ...
            try:
                ...
                categorization_response = direct_client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": categorization_prompt},
                        {"role": "user", "content": f"Analyze this query and return a JSON response: {query}"}
        ...
        if query_type in ['update_price', 'disable_item', 'enable_item']:
            if query_type == 'update_price' and item_name and new_price:
                ...
                location_ids = context.get('selected_location_ids', [1])  # Default to location 1
            ...
            location_id = context.get('selected_location_id', 1)  # Default to location 1
            location_ids = context.get('selected_location_ids', [1])  # Default to location 1
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    if callback_handler:
                        callback_handler.on_text(f"Retry {attempt+1}/{max_retries} after execution error\n")
                else:
                    raise
        ...
        if execution_result and execution_result.get('success'):
            ...
            error_msg = execution_result.get('error', 'Unknown error') if execution_result else 'Execution failed'
...
"""
        ...
        try:
            ...
            try:
                ...
                summarization_response = direct_client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": system_prompt},
                        {"role": "user", "content": enhanced_summary_prompt}
