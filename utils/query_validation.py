...
def check_syntax_errors(sql_query: str) -> List[str]:
    ...
    if open_count != close_count:
        issues.append(f"Mismatched parentheses: {open_count} opening vs {close_count} closing")
    ...
    common_keywords = ["SELECT", "FROM", "WHERE", "GROUP BY", "ORDER BY", "HAVING", "JOIN", "LIMIT"]
    for keyword in common_keywords:
        # Look for near-matches like "SLECT" or "GROUPE BY"
        if keyword == "GROUP BY" and "GROUPE BY" in sql_query.upper():
            issues.append(f"Possible typo: 'GROUPE BY' should be 'GROUP BY'")
        elif keyword == "ORDER BY" and "ORDRE BY" in sql_query.upper():
            issues.append(f"Possible typo: 'ORDRE BY' should be 'ORDER BY'")
        elif keyword == "SELECT" and re.search(r'\bSLECT\b|\bSELCT\b', sql_query.upper()):
            issues.append(f"Possible typo in 'SELECT' keyword")
...
def check_time_issues(sql_query: str) -> List[str]:
    ...
    if ("AT TIME ZONE" not in sql_query and 
        ("date" in sql_query.lower() or 
         "time" in sql_query.lower() or 
         "updated_at" in sql_query.lower() or 
         "created_at" in sql_query.lower())):
    ...
    for match in interval_matches:
        ...
        if unit.lower() in ['day', 'days'] and value > 3650:
            issues.append(f"Suspiciously large interval of {value} days (over 10 years)")
        elif unit.lower() in ['hour', 'hours'] and value > 87600:
            issues.append(f"Suspiciously large interval of {value} hours (over 10 years)")
...
def provide_query_improvement_suggestions(sql_query: str) -> List[str]:
    ...
    if "updated_at" in sql_query.lower() and "INTERVAL" not in sql_query.upper():
        suggestions.append("Consider adding a time range filter with INTERVAL for time-series data")
    
    # Check for case handling in value clauses
    if "LOWER(" not in sql_query.upper() and "UPPER(" not in sql_query.upper() and "LIKE" in sql_query.upper():
        suggestions.append("Consider using LOWER() or UPPER() for case-insensitive string comparisons")
