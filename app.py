...
try:
    ...
    logger.warning("Could not delete previous log file - it might be open in another process")
...
debug_mode = st.sidebar.checkbox("Enable Debug Mode", value=False)  # Set to False by default
...
def run_chat_sequence(messages: List[Dict[str, str]], functions: List[Dict[str, Any]], openai_client, xai_client) -> Dict[str, str]:
    ...
    try:
        ...
        logger.info(f"Using location timezone: {location_tz.zone}, User timezone: {user_tz.zone}")
        ...
        if function_call is None:
            ...
            return {"role": "assistant", "content": "I need more details to help with that."}
        ...
        if request_type in ["query_orders", "query_menu"]:
            ...
            sql_query = generate_sql_from_user_query(user_message, location_id, base_sql_query=base_sql_query)
            ...
            show_details = any(keyword in user_message.lower() for keyword in ["detail", "list", "show me", "display"])
            ...
            return {"role": "assistant", "content": "Sorry, I couldn't understand your request."}
...
def format_order_response(summary: str, results: List[Dict]) -> Dict[str, str]:
    ...
    if not results:
        ...
        for o in results:
            ...
            customer = f"{o.get('customer_first_name', '')} {o.get('customer_last_name', '')}".strip() or "N/A"
            order_date = o.get("order_created_at", "N/A")
            if isinstance(order_date, datetime):
                order_date = order_date.strftime("%Y-%m-%d %H:%M:%S")
            total_revenue = f"${o.get('order_total', 0):.2f}"
            phone = o.get("phone", "N/A")
            table_lines.append(f"| {order_id} | {customer} | {order_date} | {total_revenue} | {phone} |")
        table_text = "\n".join(table_lines)
...
def process_query_results(query_result: Dict[str, Any], user_question: str, openai_client, xai_client) -> str:
    """
    ...
    if query_result['success']:
        try:
            ...
            count_value = results[0]['count'] if results else 0  # This gets the 14 from your SQL result
            ...
            response = requests.post(xai_client["XAI_API_URL"], headers=headers, json=data)
            ...
            try:
                ...
                final_message = f"Total orders: {count_value}. Detailed order information is available below." if results else "No orders to display."
...
if __name__ == "__main__":
    ...
    if (prompt := st.chat_input("What do you want to know?")) is not None:
        ...
        logger.debug(f"Chat history updated. full_chat_history: {st.session_state.full_chat_history}, api_chat_history: {st.session_state.api_chat_history}")
